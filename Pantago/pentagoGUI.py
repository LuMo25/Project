#importation des programmes et modules qui nous serviront ? l'avenir
import pygame, sys
from pygame import *
from bases import *
...
def test_ring(n):
    ...
    return Ring
#test qui permet de choisir les panneaux de progressions ? la fin du jeu
def test_neon(n):
    if n==6:
...
#procedure qui jourera l'affichage du fond ?cran avec des boutons de fin ainsi que des sons, et des images, indiquant qui ? gagn?
def win(joueur,n):
...
#permet de v?rifier si une case est libre lorqu'on fera appelle ? cette fonction
def verif(plateau,c,l,joueur) :
    if plateau[l][c] != 0:
        return False
    else :
        return True
#permet de changer une valeur de la liste bidimensionelle repr?sentant notre plateau
def change(plateau,c,l,joueur) :
    plateau[l][c] = joueur
    return plateau
#dessinera les pions apr?s un raffr?chissement complet de notre plateau (les cadrant ayant une tous une texture diff?rente) en fonction de la taille de celui ci
...
#cr?era une animation et un effet sonore l'orsqu'on quittera le jeu
...
#permet de g?rer la fa?on dont on pose le pion
def Pose_pion(joueur,plateau,n,C):
    #test les coordonn?s pour choisir o? l'on veut faire appara?tre un pion
    for ligne in range(n):
            ...
            for k in range (n):
                if ((k*C)+112.5)//1 <= cy <= (((k+1)*C)+112.5)//1:
                    ...
                    if libre:
                        ...
                        dessine(plateau,C,n)
                        return 1
#permet de g?rer la fa?on dont on fait la rotation
def Rotation(plateau,joueur,n,C):
    #coordonn?s des bouttons pour pivoter un cadrant
    if 132 <= cx <= 431:
        if 19 <= cy <= 93:
            e=1
            c=1
    ...
    if 807 <= cx <= 881:
        if 469 <= cy <= 768:
            ...
            dessine(plateau,C,n)
            return 1
#permet de g?rer le programme sur la gestion des tours d'un joueur
...
#ces images montront ? qui doit jouer: orange pour le joueur orange et bleu pour le joueur bleu (logique)
...
#image qui permettre de faire tourner le cadrant vers la gauche (garde un aspect d?corative, et est de couleur bleut? comme son nom l'indique)
...
#idem que pour LB mais pivot? de 90?
LGB = pygame.image.load('light_blue01_01.png')
#idem que pour LO mais pivot? de 90?
...
#fond d'?crant final lorsqu'on quitte ou qu l'on gagne (ou perd ou ni l'un ni l'autre...)
BGF=pygame.image.load('bg_f_00_00_00.png')
#image de l'animation (elle restera inanim? tant qu'on ne quittera pas le jeu)
door=pygame.image.load('metal_door_anime00_12.png')
#permet de suprimer le 'film' (comprendre film d'une p?licule photo) d?passant de l'image d'animation
...
#fond d'?crant pour selectionner les niveaux (avec ses boutton)
BGLS=image.load('background_level_selector_00_00.png')
#r?gle du jeu (pour bien comprendre comment jouer)
rule=image.load('rule_00_00_00.png')
#pr?paration de l'affichage du jeu et de son initialisation
...
#permettent de cr?er un ordre lors d'un tour de jeu
tour=0
compteur=0
#ne fonctionnera pas bien avec joueur 1
joueur=2
#variable servant ? fermer notre boucle de jeu
inProgress = True
#son d'embiance jou?
jouerSonlvl6.play(-1)
#servira pour savoir si un joueur ? gagn? et permettera d'?viter de pouvoir continuer ? poser des pions (ou m?me de quitter le jeu sans le voloire lorsqu'on cliquera sur une case prise)
wins=1
#permet de savoir si le menu de selection des niveau est ouvert
SL=0
C=675/n

#idem que pour savoir si un joueur ? gagn? sauf que celle ci permet de savoir si le menu d'option est ouvert
menus=0
#d?marrage
while inProgress:
    for event in pygame.event.get():
        ...
        if event.type == MOUSEBUTTONUP:
            cx,cy = event.pos
            #on veut que le menu de selection des niveau souvre ? la fin du jeu, on utilise SL pour l'ouvrire
            if wins==0 and 544 <= cx <= 1115 :
                if 180 <= cy <= 339 :
                    SL=1
                    plus6.play()
            #coordonn? du boutton pour quitter du menu d'option
            if menus==1 and 1046 <= cx <= 1441 :
                if 84 <= cy <= 281 :
                    plus5.play()
                    seeyousoon.play()
                    #m?me princique que plus haut
                    inProgress=False
            #affichage de la r?gle du jeu depuis le menu d'option
            if menus==1 and 1046 <= cx <= 1441 :
                if 609 <= cy <= 817:
                    plus5.play()
                    maSurface.blit(rule,(700,0))
                    maSurface.blit(close,(1516,18))
            #coordonn? de l'iconne d'option
            if menus==0 and wins==1 and 1516 <= cx <= 1579 :
                if 18 <= cy <= 81 :
                    ...
                    maSurface.blit(close,(1516,18))
                    menus=1
            #on utilise un elif pour eviter de fermer puis r?ouvrir le menu d'option
            elif menus==1 and 1516 <= cx <= 1579 :
                if 18 <= cy <= 81 :
                    plus4.play()
                    close1(joueur)
                    menus=0
            #coordonn? du boutton pour quitter lors de la fin du jeu
            elif wins==0 and SL==0 and 544 <= cx <= 1115 :
                if 418 <= cy <= 577 :
                    plus6.play()
                    seeyousoon.play()
                    #on arr?te la boucle et le jeu se ferme par l'animation en fin de programme
                    inProgress=False
            #coordonn?es du boutton rejou? qui r?initialisera les valeurs de note liste bidimantionnelle et r?afichera notre plateau
            if wins==0 and SL==0 and 544 <= cx <= 1115 :
                if 660 <= cy <= 819 :
                    ...
                    wins=1
                    replaydef(n)
            #permet de tester une action sur le jeu et de voir si on ? gagn?
            else:
                if menus==0 and wins==1:
                    ...
                    if tour == 1:
                        ...
                        if not victoire:
                            ...
                            win(joueur,n)
                            wins=0
            #test permettant d'afficher le menu de selection et d'arr?ter la musique d'ambiance
            if menus==1 and 1046 <= cx <= 1441 :
                if 354 <= cy <= 551 :
                    SL=1
                    wins=0
